...
const driversService = async (limit) => {
    if (limit) {
        return await db_1.db.query.driversTable.findMany({
            limit: limit,
        });
    }
    return await db_1.db.query.driversTable.findMany();
...
const getDriverService = async (id) => {
    return await db_1.db.query.driversTable.findFirst({
...
const createDriver = async (driver) => {
    await db_1.db.insert(schema_1.driversTable).values(driver);
    return 'Driver created successfully';
};
exports.createDriver = createDriver;
// update a driver in the database
const updateDriver = async (id, driver) => {
    await db_1.db.update(schema_1.driversTable).set(driver).where((0, drizzle_orm_1.eq)(schema_1.driversTable.id, id));
    return 'Driver updated successfully';
};
exports.updateDriver = updateDriver;
// delete a driver from the database
const deleteDriver = async (id) => {
    await db_1.db.delete(schema_1.driversTable).where((0, drizzle_orm_1.eq)(schema_1.driversTable.id, id));
