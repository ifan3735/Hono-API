...
const ordersStatusService = async (limit) => {
    if (limit) {
        return await db_1.db.query.ordersStatusTable.findMany({
            limit: limit,
        });
    }
    return await db_1.db.query.ordersStatusTable.findMany();
...
const getOrdersStatusService = async (id) => {
    return await db_1.db.query.ordersStatusTable.findFirst({
...
const createOrdersStatus = async (ordersStatus) => {
    await db_1.db.insert(schema_1.ordersStatusTable).values(ordersStatus);
...
const updateOrdersStatus = async (id, ordersStatus) => {
    await db_1.db.update(schema_1.ordersStatusTable).set(ordersStatus).where((0, drizzle_orm_1.eq)(schema_1.ordersStatusTable.id, id));
...
const deleteOrdersStatus = async (id) => {
    await db_1.db.delete(schema_1.ordersStatusTable).where((0, drizzle_orm_1.eq)(schema_1.ordersStatusTable.id, id));
